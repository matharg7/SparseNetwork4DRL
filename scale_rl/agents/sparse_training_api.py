...
class PolynomialScheduleDST(sparsity_schedules.PolynomialSchedule):
  ...
  def is_mask_update_iter(self, step):
    if step is None:
      # This initial check is fine, as it happens before JIT tracing.
      return False


    # Use jax.lax.cond for logic that depends on the `step` tracer.
    return jax.lax.cond(
        # 1. The condition (can be a tracer)
        step >= self.update_end_step,
        
        # 2. Function to run if condition is True
        lambda s: s % self.update_freq == 0,
        
        # 3. Function to run if condition is False
        lambda s: jnp.logical_and(
            s % self.update_freq_dst == 0, 
            s >= self.update_start_step + self.update_freq
        ),
        
        # 4. The operand(s) to pass to the functions
        step
    )
...
def create_updater_from_custom_config(
    ...
    if config.algorithm in CUSTOM_ALGORITHM_REGISTRY:
        ...
        # del config.algorithm
            ...
            # config.drop_fraction_fn = optax.cosine_decay_schedule(
            #     0.90, config.update_end_step//0.8
            # )
            # config.drop_fraction_fn = optax.warmup_cosine_decay_schedule(
            #     init_value=0.1,
            #     peak_value=config.get("drop_fraction", 0.1),
            #     warmup_steps=config.update_end_step,
            #     decay_steps=config.update_end_step//0.8,
            #     end_value=0.0,
            # )
            gmp_dst_schedule = optax.cosine_decay_schedule(
                0.90, config.update_end_step
            )
            dst_schedule = optax.constant_schedule(0.3)
            config.drop_fraction_fn = optax.join_schedules(
                schedules=[gmp_dst_schedule, dst_schedule],
                boundaries=[config.update_end_step]
            )
